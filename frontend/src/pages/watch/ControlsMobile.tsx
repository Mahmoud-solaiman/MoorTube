import { useState } from 'react';
import { ControlsProps } from '../../types/types';
import './ControlsMobile.scss';
import { PlayButton, useMediaState, FullscreenButton, Time } from '@vidstack/react';
import VideoPlayerSettings from './VideoPlayerSettings';
import PlaySpeedControls from '../../components/PlaySpeedControls';
function ControlsMobile({
  isBlur,
  setIsBlur,
  isShowControls,
  setBlurBoxes,
  blurBoxes,
  hideControls
}: ControlsProps) {
  const isPaused = useMediaState('paused');
  const isFullScreen = useMediaState('fullscreen');
  const [isSettings, setIsSettings] = useState<boolean>(false);
  const [isSpeedSettings, setIsSpeedSettings] = useState<boolean>(false);

  return (
    <div onTouchEnd={e => e.stopPropagation()} className={isShowControls ? "controls-container-mobile" : "controls-container-mobile hidden"} >
      <div className="custom-controls" onClick={() => {
          if(hideControls) hideControls();
        }}>
        <div
          aria-label="blur toggle"
          title="blur toggle"
          className={isBlur ? "blur-toggle blurred" : "blur-toggle"}
          onClick={() => setIsBlur(!isBlur)}
        >
          <div aria-hidden="true"></div>
        </div>

        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640">
          <path d="M512 176C520.8 176 528 183.2 528 192L528 448C528 456.8 520.8 464 512 464L128 464C119.2 464 112 456.8 112 448L112 192C112 183.2 119.2 176 128 176L512 176zM128 128C92.7 128 64 156.7 64 192L64 448C64 483.3 92.7 512 128 512L512 512C547.3 512 576 483.3 576 448L576 192C576 156.7 547.3 128 512 128L128 128zM216 272L248 272C252.4 272 256 275.6 256 280C256 293.3 266.7 304 280 304C293.3 304 304 293.3 304 280C304 249.1 278.9 224 248 224L216 224C185.1 224 160 249.1 160 280L160 360C160 390.9 185.1 416 216 416L248 416C278.9 416 304 390.9 304 360C304 346.7 293.3 336 280 336C266.7 336 256 346.7 256 360C256 364.4 252.4 368 248 368L216 368C211.6 368 208 364.4 208 360L208 280C208 275.6 211.6 272 216 272zM384 280C384 275.6 387.6 272 392 272L424 272C428.4 272 432 275.6 432 280C432 293.3 442.7 304 456 304C469.3 304 480 293.3 480 280C480 249.1 454.9 224 424 224L392 224C361.1 224 336 249.1 336 280L336 360C336 390.9 361.1 416 392 416L424 416C454.9 416 480 390.9 480 360C480 346.7 469.3 336 456 336C442.7 336 432 346.7 432 360C432 364.4 428.4 368 424 368L392 368C387.6 368 384 364.4 384 360L384 280z" />
        </svg>

        <div className="settings-btn" title="settings">
          <svg className={(isSettings || isSpeedSettings) ? "settings-toggled" : undefined} onClick={() => {
            
            if (isSpeedSettings) {
              setIsSettings(false);
              setIsSpeedSettings(false);
            } else {
              setIsSettings(!isSettings);
              setIsSpeedSettings(false);
            }

          }} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640">
            <path d="M259.1 73.5C262.1 58.7 275.2 48 290.4 48L350.2 48C365.4 48 378.5 58.7 381.5 73.5L396 143.5C410.1 149.5 423.3 157.2 435.3 166.3L503.1 143.8C517.5 139 533.3 145 540.9 158.2L570.8 210C578.4 223.2 575.7 239.8 564.3 249.9L511 297.3C511.9 304.7 512.3 312.3 512.3 320C512.3 327.7 511.8 335.3 511 342.7L564.4 390.2C575.8 400.3 578.4 417 570.9 430.1L541 481.9C533.4 495 517.6 501.1 503.2 496.3L435.4 473.8C423.3 482.9 410.1 490.5 396.1 496.6L381.7 566.5C378.6 581.4 365.5 592 350.4 592L290.6 592C275.4 592 262.3 581.3 259.3 566.5L244.9 496.6C230.8 490.6 217.7 482.9 205.6 473.8L137.5 496.3C123.1 501.1 107.3 495.1 99.7 481.9L69.8 430.1C62.2 416.9 64.9 400.3 76.3 390.2L129.7 342.7C128.8 335.3 128.4 327.7 128.4 320C128.4 312.3 128.9 304.7 129.7 297.3L76.3 249.8C64.9 239.7 62.3 223 69.8 209.9L99.7 158.1C107.3 144.9 123.1 138.9 137.5 143.7L205.3 166.2C217.4 157.1 230.6 149.5 244.6 143.4L259.1 73.5zM320.3 400C364.5 399.8 400.2 363.9 400 319.7C399.8 275.5 363.9 239.8 319.7 240C275.5 240.2 239.8 276.1 240 320.3C240.2 364.5 276.1 400.2 320.3 400z" />
          </svg>
        </div>

        {
          isSettings &&
          <VideoPlayerSettings
            setIsSpeedSettings={setIsSpeedSettings}
            setIsSettings={setIsSettings}
            setBlurBoxes={setBlurBoxes}
            blurBoxes={blurBoxes}
          />
        }

        {
          isSpeedSettings &&
          <PlaySpeedControls
            setIsSpeedSettings={setIsSpeedSettings}
            setIsSettings={setIsSettings}
          />
        }
      </div>

      <div className="play-controls">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640">
          <path d="M491 100.8C478.1 93.8 462.3 94.5 450 102.6L192 272.1L192 128C192 110.3 177.7 96 160 96C142.3 96 128 110.3 128 128L128 512C128 529.7 142.3 544 160 544C177.7 544 192 529.7 192 512L192 367.9L450 537.5C462.3 545.6 478 546.3 491 539.3C504 532.3 512 518.8 512 504.1L512 136.1C512 121.4 503.9 107.9 491 100.9z" />
        </svg>
        <PlayButton>

          {
            isPaused ?
              <svg className="pause-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640">
                <title>pause icon</title>
                <path d="M187.2 100.9C174.8 94.1 159.8 94.4 147.6 101.6C135.4 108.8 128 121.9 128 136L128 504C128 518.1 135.5 531.2 147.6 538.4C159.7 545.6 174.8 545.9 187.2 539.1L523.2 355.1C536 348.1 544 334.6 544 320C544 305.4 536 291.9 523.2 284.9L187.2 100.9z" />
              </svg> :
              <svg className="play-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640">
                <title>play icon</title>
                <path d="M176 96C149.5 96 128 117.5 128 144L128 496C128 522.5 149.5 544 176 544L240 544C266.5 544 288 522.5 288 496L288 144C288 117.5 266.5 96 240 96L176 96zM400 96C373.5 96 352 117.5 352 144L352 496C352 522.5 373.5 544 400 544L464 544C490.5 544 512 522.5 512 496L512 144C512 117.5 490.5 96 464 96L400 96z" />
              </svg>
          }

        </PlayButton>

        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640">
          <path d="M149 100.8C161.9 93.8 177.7 94.5 190 102.6L448 272.1L448 128C448 110.3 462.3 96 480 96C497.7 96 512 110.3 512 128L512 512C512 529.7 497.7 544 480 544C462.3 544 448 529.7 448 512L448 367.9L190 537.5C177.7 545.6 162 546.3 149 539.3C136 532.3 128 518.7 128 504L128 136C128 121.3 136.1 107.8 149 100.8z" />
        </svg>
      </div>
      <div className="player-controls">
        <div className="video-time-mobile">
          <Time type="current" /> &#x2F; <Time type="duration" />
        </div>

        <FullscreenButton>

          {
            isFullScreen ?
              <svg className="exit-fullscreen" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640">
                <path d="M256 128C256 110.3 241.7 96 224 96C206.3 96 192 110.3 192 128L192 192L128 192C110.3 192 96 206.3 96 224C96 241.7 110.3 256 128 256L224 256C241.7 256 256 241.7 256 224L256 128zM128 384C110.3 384 96 398.3 96 416C96 433.7 110.3 448 128 448L192 448L192 512C192 529.7 206.3 544 224 544C241.7 544 256 529.7 256 512L256 416C256 398.3 241.7 384 224 384L128 384zM448 128C448 110.3 433.7 96 416 96C398.3 96 384 110.3 384 128L384 224C384 241.7 398.3 256 416 256L512 256C529.7 256 544 241.7 544 224C544 206.3 529.7 192 512 192L448 192L448 128zM416 384C398.3 384 384 398.3 384 416L384 512C384 529.7 398.3 544 416 544C433.7 544 448 529.7 448 512L448 448L512 448C529.7 448 544 433.7 544 416C544 398.3 529.7 384 512 384L416 384z" />
              </svg> :
              <svg className="fullscreen" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640">
                <path d="M128 96C110.3 96 96 110.3 96 128L96 224C96 241.7 110.3 256 128 256C145.7 256 160 241.7 160 224L160 160L224 160C241.7 160 256 145.7 256 128C256 110.3 241.7 96 224 96L128 96zM160 416C160 398.3 145.7 384 128 384C110.3 384 96 398.3 96 416L96 512C96 529.7 110.3 544 128 544L224 544C241.7 544 256 529.7 256 512C256 494.3 241.7 480 224 480L160 480L160 416zM416 96C398.3 96 384 110.3 384 128C384 145.7 398.3 160 416 160L480 160L480 224C480 241.7 494.3 256 512 256C529.7 256 544 241.7 544 224L544 128C544 110.3 529.7 96 512 96L416 96zM544 416C544 398.3 529.7 384 512 384C494.3 384 480 398.3 480 416L480 480L416 480C398.3 480 384 494.3 384 512C384 529.7 398.3 544 416 544L512 544C529.7 544 544 529.7 544 512L544 416z" />
              </svg>
          }

        </FullscreenButton>
      </div>
    </div>
  );
}

export default ControlsMobile;